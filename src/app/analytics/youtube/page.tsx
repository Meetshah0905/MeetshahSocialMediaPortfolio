...
export const metadata = {
  title: "YouTube Insights Archive — Meet Shah",
  description: "Browse verified YouTube subscribers growth, views, watch time, and campaign demographics for Meet Shah.",
};

export default async function YouTubeArchivePage() {
  const reports = await listReports("youtube_main");
      ...
      <WhiteAtmosphereSection grid={true} halo="both" className="pt-24 lg:pt-32 pb-12 bg-white border-b border-border">
        <div className="max-w-3xl flex flex-col items-start text-left space-y-4">
          ...
          <h1 className="font-heading text-4xl sm:text-5xl font-bold tracking-tight text-ink leading-tight">
            Verified YouTube Insights
          </h1>
          <p className="text-xs sm:text-sm text-body max-w-[55ch] leading-relaxed">
            Historical performance logs tracking subscribers, views, watch time, and click-through rates.
          </p>
        </div>
                    ...
                    <div className="flex gap-4 items-start">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1.5 mt-2 text-xs text-body">
                          ...
                          <span>•</span>
                          <span>Subscribers: {report.metrics.subscribers?.toLocaleString() ?? "N/A"}</span>
                        </div>
                      </div>
                    </div>

                    <div className="flex flex-wrap gap-3 w-full sm:w-auto">
                      <ArrowPillButton href={`/analytics/youtube/${report.id}`} size="md">
                        View Details
                      </ArrowPillButton>
                      <a
