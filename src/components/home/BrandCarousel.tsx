...
export function BrandCarousel() {
  ...
  return (
    <section className="py-16 bg-slate-950/90 border-y border-white/10 relative overflow-hidden backdrop-blur-md">
      {/* Background radial glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[200px] bg-red-600/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="relative w-full overflow-hidden py-4 group">
        ...
        <div className="flex gap-6 sm:gap-8 w-max animate-marquee group-hover:[animation-play-state:paused]">
          {marqueeItems.map((brand, idx) => (
            <div
              key={`${brand.name}-${idx}`}
              className="flex items-center justify-center h-24 sm:h-28 px-8 sm:px-10 bg-white/95 backdrop-blur-sm border border-slate-200/80 rounded-2xl shadow-lg hover:shadow-2xl hover:scale-105 hover:bg-white transition-all duration-300 min-w-[180px] sm:min-w-[220px] group/card"
            >
              <div className="relative w-36 sm:w-44 h-14 sm:h-16 flex items-center justify-center">
                <Image
                  ...
                  alt={`${brand.name} logo`}
                  fill
                  className="object-contain p-1 filter drop-shadow-sm group-hover/card:scale-105 transition-all duration-300"
                />
              </div>
            </div>
          ))}
