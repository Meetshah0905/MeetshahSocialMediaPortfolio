...
export function UGCHero() {
  ...
  return (
    <section
      ref={heroRef}
      id="ugc-hero"
      className="relative w-full bg-[#050811] overflow-clip flex flex-col items-center select-none pt-2 md:pt-4"
    >
      ...
      <div
        ref={stageMaskRef}
        className="w-full max-w-[1400px] px-3 sm:px-6 relative flex flex-col justify-center items-center z-10 space-y-6"
      >
        <div
          ref={stageRef}
          className="relative w-full overflow-hidden rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.5),0_0_80px_rgba(220,60,80,0.15)] border border-white/10 p-2 bg-slate-950/40 backdrop-blur-xs"
        >
      ...
      <div className="relative w-full max-w-[1400px] z-20 mt-5 px-6">
        <Container className="flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div ref={ctasRef} className="flex gap-3 w-full sm:w-auto">
            <ArrowPillButton href="/contact?vertical=ugc" size="md" className="flex-1 sm:flex-initial">
